...
ax = axes[0, 1]
bars = ax.barh(
    summary["Model"], summary["F1"], color=sns.color_palette("rocket", len(summary))
)
...
for idx, row in summary.iterrows():
    ax.annotate(
        row["Model"], (row["Recall"], row["Precision"]), fontsize=9, ha="center"
    )
...
ax.bar(
    ...
    summary["CV ROC-AUC"],
    yerr=summary["CV Std"],
    color=sns.color_palette("mako", len(summary)),
    capsize=5,
    alpha=0.7,
)
ax.set_xticks(x_pos)
...
metrics_df = summary.set_index("Model")[
    ["Test Accuracy", "Precision", "Recall", "F1", "ROC-AUC", "Specificity"]
]
...
fig.suptitle(
    f"Feature Importance Analysis - {best_name}", fontsize=16, fontweight="bold"
)
...
ax.axhline(
    y=cumsum.iloc[-1] * 0.8, color="r", linestyle="--", label="80% of total importance"
)
ax.legend()
